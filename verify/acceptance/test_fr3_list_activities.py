...
def test_list_activities_success(client, user_ids):
    ...
    payloads = [
        {
            "user_id": u1, "sport_type": "run",
            "start_time": "2025-06-25T08:00:00Z", "elapsed_time": 1800,
            "distance_m": 5000.0, "polyline": [[40.71, -74.00, 10.0, 1719907200]],
        },
        {
            "user_id": u1, "sport_type": "ride",
            "start_time": "2025-06-25T10:00:00Z", "elapsed_time": 3600,
            "distance_m": 25000.0, "polyline": [[40.72, -74.01, 15.0, 1719914400]],
        },
    ]
    ids = []
    for p in payloads:
...
def test_list_activities_pagination(client, user_ids):
    ...
    for i in range(5):
        p = {
            "user_id": u1, "sport_type": "walk",
            "start_time": f"2025-06-25T0{i}:00:00Z", "elapsed_time": 600,
            "distance_m": 1000.0, "polyline": [[40.71, -74.00, 5.0, 1719907200]],
        }
...
def test_list_activities_nonexistent_user_404(client):
    ...
    assert_404(client.get("/users/00000000-0000-0000-0000-000000000000/activities?limit=10&offset=0"))
